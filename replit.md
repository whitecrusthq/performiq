...
This project is a pnpm workspace monorepo utilizing TypeScript for both frontend and backend development. It comprises two main applications: PerformIQ, a performance management system, and HiraCRM, a customer relationship management system. Both applications are designed to be fully self-contained, without shared library packages.

PerformIQ aims to streamline employee performance appraisals, goal setting, and attendance tracking. HiraCRM focuses on unified multi-channel customer communication, AI-assisted responses, campaign management, and customer intelligence.
...
- **TypeScript**: 5.9
- **Database**: PostgreSQL with Drizzle ORM (PerformIQ) and Sequelize (HiraCRM)
- **Validation**: Zod
...
- **PostgreSQL**: Primary database for both PerformIQ (via Drizzle ORM) and HiraCRM (via Sequelize).
...
- **AI Providers**: Gemini, OpenAI, Anthropic Claude, and custom OpenAI-compatible endpoints for HiraCRM's AI features.
- **Mailgun**: Used by HiraCRM for email broadcasting and campaign management.
- **Meta Webhooks**: For WhatsApp Business API, Facebook Messenger, and Instagram Direct integration within HiraCRM.
- **Google Maps**: For displaying GPS coordinates in attendance logs in HiraCRM.