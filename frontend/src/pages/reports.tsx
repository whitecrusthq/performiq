...
import { BarChart3, Download, Users, CheckCircle2, TrendingUp, FileSpreadsheet, Building2, ChevronDown } from "lucide-react";
...
type EmployeeRow = { id: number; name: string; jobTitle: string | null; role: string; totalAppraisals: number; completed: number; completionRate: number; avgScore: number | null };
...
export default function Reports() {
  ...
  const handleExportExcel = async () => {
    ...
    // Department employees (when filtered)
    ...
    const fileName = selectedDept
      ? `PerformIQ_${selectedDept.replace(/\s+/g, "_")}_Report.xlsx`
      : "PerformIQ_Report.xlsx";
  ...
  const handleExportPDF = async () => {
    ...
    if (selectedDept && data.employeeList.length > 0) {
      ...
      autoTable(doc, {
        ...
        body: data.employeeList.map(e => [
          e.name, e.jobTitle ?? "—", e.role,
          e.totalAppraisals, e.completed,
          `${e.completionRate}%`,
          e.avgScore?.toFixed(2) ?? "N/A",
        ]),
        theme: "striped",
      });
      y = (doc as any).lastAutoTable.finalY + 8;
    ...
    const fileName = selectedDept
      ? `PerformIQ_${selectedDept.replace(/\s+/g, "_")}_Report.pdf`
      : "PerformIQ_Report.pdf";
    doc.save(fileName);
  };
  ...
  return (
    <div>
      <PageHeader title="Reports & Analytics" description="Performance insights across the organisation.">
      ...
      {selectedDept && (
        <div className="mb-4 flex items-center gap-2">
          ...
          <button onClick={() => handleDeptChange("")} className="text-xs text-muted-foreground underline hover:text-foreground">
            Clear filter
          </button>
        </div>
      )}
      ...
      {!loading && !error && data && (
        <>
          ...
          {/* Department employee table (only when dept filter active) */}
                    ...
                    {data.employeeList.map(e => (
                      <tr key={e.id} className="hover:bg-muted/30">
                        ...
                        <td className="py-2.5 pr-4 text-right">
                          <span className={`font-semibold ${e.completionRate === 100 ? 'text-green-600' : e.completionRate >= 50 ? 'text-amber-600' : 'text-red-500'}`}>
                            {e.completionRate}%
                          </span>
                        </td>
                        <td className="py-2.5 text-right font-medium">
                          {e.avgScore !== null ? (
                            <span className={e.avgScore >= 4 ? 'text-green-600' : e.avgScore >= 2.5 ? 'text-amber-600' : 'text-red-500'}>
          ...
          {/* Avg score by dept chart (hide when filtering a specific dept) */}
