...
async function seed() {
  try {
    ...
    const { rows: [{ id: cycleId }] } = await pool.query(`SELECT id FROM cycles WHERE status='active' LIMIT 1`);
    ...
    const critIds = critRows.map(r => r.id);

    // Appraisals for Alice and Bob
    for (const [empEmail, status] of [['alice@performiq.com','self_review'], ['bob@performiq.com','manager_review']]) {
      ...
      if (ex.rows.length === 0) {
        const { rows: [{ id: apprId }] } = await pool.query(
        ...
        for (const cId of critIds) {
          await pool.query(`INSERT INTO appraisal_scores (appraisal_id, criterion_id, self_score, self_note) VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING`,
            [apprId, cId, status === 'manager_review' ? (3 + Math.round(Math.random()*2)) : null, status === 'manager_review' ? 'Good progress' : null]);
        }
      }
    }

    // Carol - completed appraisal
    ...
    if (ex2.rows.length === 0) {
      ...
      for (const cId of critIds) {
        await pool.query(`INSERT INTO appraisal_scores (appraisal_id, criterion_id, self_score, manager_score) VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING`,
    ...
    console.log('  alice@performiq.com — Employee');
  } catch (e) {
