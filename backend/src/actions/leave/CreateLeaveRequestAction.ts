...
export class CreateLeaveRequestAction {
  static async handle(req: AuthRequest, res: Response) {
    try {
      const { leaveType, startDate, endDate, reason, approverIds, coverUserIds } = req.body;
      if (!leaveType || !startDate || !endDate) {
        res.status(400).json({ error: "leaveType, startDate, and endDate are required" }); return;
      }

      const result = await LeaveController.createLeaveRequest(req.user!.id, { leaveType, startDate, endDate, reason, approverIds, coverUserIds });
