...
export const leavePoliciesTable = pgTable("leave_policies", {
  ...
  cycleMode: text("cycle_mode").notNull().default("dates"),
  ...
  cycleDays: integer("cycle_days").notNull().default(365),
  rolloverEnabled: boolean("rollover_enabled").notNull().default(false),
  maxRolloverDays: integer("max_rollover_days").notNull().default(0),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
